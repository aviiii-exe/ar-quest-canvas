...
const Map = () => {
  ...
  return <div className="h-screen w-full relative overflow-hidden">
      ...
      {/* Full Screen Map */}
      <HeritageMap sites={sites || []} visitedSiteIds={visitedSiteIds} onSiteClick={handleSiteClick} userLocation={userLocation} fullScreen={true} />
      ...
      <div className="absolute top-20 left-4 z-40 flex flex-col gap-2">
        <Badge variant="secondary" className="text-sm py-1 px-3 bg-background/90 backdrop-blur-sm shadow-md text-secondary">
      ...
      {selectedSite && <div className="absolute bottom-0 left-0 right-0 z-40 p-4 animate-in slide-in-from-bottom">
          <Card className="max-w-md mx-auto shadow-2xl">
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="flex gap-2 mt-2">
                    {selectedSite.category && <Badge variant="outline">{selectedSite.category}</Badge>}
                    {selectedSite.difficulty && <Badge variant="secondary">{selectedSite.difficulty}</Badge>}
                  </div>
                </div>
            ...
            <CardContent className="space-y-4">
              {selectedSite.short_description && <p className="text-sm text-muted-foreground">
                  {selectedSite.short_description}
                </p>}
              
              <div className="flex flex-wrap gap-4 text-sm">
                {selectedSite.estimated_duration && <div className="flex items-center gap-1 text-muted-foreground">
                    <Clock className="h-4 w-4" />
                    {selectedSite.estimated_duration}
                  </div>}
                {selectedSite.xp_reward && <div className="flex items-center gap-1 text-primary">
              ...
              <ProximityCheckIn site={selectedSite} onCheckIn={handleProximityCheckIn} isVisited={visitedSiteIds.includes(selectedSite.id)} checkInRadiusMeters={PROXIMITY_RADIUS_METERS} />
            </CardContent>
          </Card>
        </div>}
    </div>;
